    ...
    <div class="grid grid-cols-4 sm:grid-cols-4 lg:grid-cols-4 p-4 gap-4">
        <div class="col-span-3">
            <div class="flex flex-col text-center w-full px-2 pb-2 bg-white">
                <h1 class="sm:text-xl text-lg title-font my-2 text-gray-800 text-center p-2 uppercase">Projets</h1>
                <p class="lg:w-2/3 mx-auto leading-relaxed text-base">Toutes les projets
                </p>
                <div class="w-full">
                    <table class="table-auto w-full text-left bg-colorspace-no-wrap border bg-white px-2">
                        ...
                        <tbody>
                            ...
                            @if (count($parametre) > 0)
                                @foreach ($parametre as $parametr)
                                    {{-- @if (!$project->hasRole('Root')) --}}
                                    <tr>
                                            <!-- DELETE using link -->
                                            {!! Form::open(['action' => ['ParametreController@update', $parametr->id], 'method' => 'PATCH', 'enctype' => 'multipart/form-data', 'class' => 'w-full']) !!}

                                            {{ csrf_field() }}

                                            <td class="px-2 py-3 text-sm">
                                                {!! Form::text('name', $parametr->name, ['autocomplete' => 'off','id' => 'name','class' =>'desabled']) !!}
                                               </td>

                                            <td class="px-2 py-3 text-sm">
                                             {!! Form::text('value', $parametr->value, ['autocomplete' => 'off','id' => 'name','class' =>'peer border border-main focus:outline-none focus:border-secondary shadow-sm focus:shadow-sm-sm w-full py-1 px-2 h-8 text-sm bg-bg-color']) !!}
                                            </td>
                                ...
                                <tr>
                                    <td colspan="6">
                                        <div
                                            class="p-3 flex flex-col items-center justify-center text-sm pb-6 text-center w-full h-full">
                                            No Project has been added
                                            <button id="addUparametreButtonLink" class="flex text-secondary">
        ...
        <div class="">
            <button id="addparametreButtonFileImg" class="bg-white h-auto p-4" type="button">
