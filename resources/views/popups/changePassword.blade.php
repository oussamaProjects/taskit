<div id="modalchangePassword"
    ...
    <div
        ...
        <div class="overflow-y-scroll py-4 px-8">
            ...
            <div class="mb-2">
                {{ Form::password('current_password', ['id' => 'current_password','class' =>'peer border border-main focus:outline-none focus:border-secondary shadow-sm focus:shadow-sm-sm w-full py-1 px-2 h-8 text-sm bg-bg-color']) }}
                <label for="current_password" class="text-xs opacity-75 scale-75">Mot de passe actuel</label>
                @if ($errors->has('current_password'))
                    <span class=" text-red-500 text-xs">{{ $errors->first('current_password') }}</span>
                @endif
            </div>
            <div class="mb-2">
                {{ Form::password('new_password', ['id' => 'new_password','class' =>'peer border border-main focus:outline-none focus:border-secondary shadow-sm focus:shadow-sm-sm w-full py-1 px-2 h-8 text-sm bg-bg-color']) }}
                <label for="new_password" class="text-xs opacity-75 scale-75">Nouveau mot de passe</label>
                @if ($errors->has('new_password'))
                    <span class=" text-red-500 text-xs">{{ $errors->first('new_password') }}</span>
                @endif
            </div>
            <div class="mb-2">
                {{ Form::password('new_password_confirmation', ['id' => 'new_password_confirmation','class' =>'peer border border-main focus:outline-none focus:border-secondary shadow-sm focus:shadow-sm-sm w-full py-1 px-2 h-8 text-sm bg-bg-color']) }}
                <label for="new_password_confirmation" class="text-xs opacity-75 scale-75">Confirmez le mot de
                    passe</label>
                @if ($errors->has('new_password_confirmation'))
                    <span class=" text-red-500 text-xs">{{ $errors->first('new_password_confirmation') }}</span>
                @endif
            </div>
