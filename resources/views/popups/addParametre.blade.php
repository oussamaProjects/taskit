<div id="modelparametre"
    ...
    <div class="max-h-2/3 w-1/2 bg-bg-color overflow-y-scrollborder-1 border-main shadow-sm rounded-sm flex flex-col items-stretch justify-center relative">
        ...
        <button id="closebuttonparametre" type="button" class="focus:outline-none float-right">
        ...
        <div class="overflow-y-scroll py-4 px-8">
            ...
            {!! Form::open(['action' => 'ParametreController@store', 'method' => 'POST', 'class' => '']) !!}
            ...
            <div class="mb-2 relative">
                <label for="name" class="text-xs opacity-75 scale-75">
                    Value</label>
                {{ Form::text('value', '', ['autocomplete' => 'off','id' => 'name','class' =>'peer border border-main focus:outline-none focus:border-secondary shadow-sm focus:shadow-sm-sm w-full py-1 px-2 h-8 text-sm bg-bg-color']) }}
            </div>
