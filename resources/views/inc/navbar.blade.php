...
<div class="fixed w-full flex items-center justify-between h-14 text-bg-color z-10">
    ...
    @if (!Auth::guest())
        <div class="flex items-center justify-start md:justify-center w-14 md:w-64 h-14 bg-gray-800   border-none">
    ...
    <div class="flex justify-between items-center h-14 text-tiny bg-gray-800 header-right">
                ...
                <li>
                    <a href="/requests" class="flex items-center mr-4">
                        <span class="inline-flex mr-1 relative">
                            ...
                            <span
                                class="absolute w-5 h-5 rounded-full flex items-center justify-center bg-amber text-bg-color text-xs -top-2 -right-2 p-2">{{ $requests }}</span>
                        </span>
                        <span class="ml-2">
                ...
                <li>
                    @if ($trashfull > 0)
                        <a href="/trash" class="flex items-center">
                            <span class="inline-flex mr-1 relative">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 m-1 text-amber" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                                <span
                                    class="absolute w-5 h-5 rounded-full flex items-center justify-center bg-amber text-bg-color text-xs -top-2 -right-2 p-2">{{ $trashfull }}</span>
                            </span>
                        </a>
                    @else
